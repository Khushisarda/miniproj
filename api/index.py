...
import { doc, getDoc, setDoc, updateDoc, arrayUnion, arrayRemove } from "https://www.gstatic.com/firebasejs/12.12.0/firebase-firestore.js";
    ...
    grid.innerHTML = myFriends.map(user => {
        ...
        <div class="friend-card">
            ...
            <div style="position: absolute; top: 0.75rem; right: 0.75rem; display: flex; gap: 0.5rem;">
                <button class="compare-btn" onclick="openCompareModal('${user.username}')" style="padding: 0.4rem 0.6rem; background: var(--bg3); border: 1px solid var(--border2); border-radius: 6px; color: var(--text2); font-size: 0.7rem; cursor: pointer;">⚔️ Compare</button>
                <button class="remove-btn" onclick="removeFriend('${user.username}')" style="padding: 0.4rem 0.6rem; background: var(--bg3); border: 1px solid rgba(248,113,113,0.3); border-radius: 6px; color: var(--hard); font-size: 0.7rem; cursor: pointer;" title="Remove Friend">❌</button>
            </div>
        </div>`;
    }).join('');
}
...
function renderChallenges() {
    ...
    if (userChallenges.my_challenge) {
        ...
        const myProfile = allLeetcodeData.find(u => u.username === userData.my_leetcode_id) || {};
        const t = getTotals(myProfile);
        const currentSolved = c.difficulty === 'Any' ? t.total : (t[c.difficulty.toLowerCase()] || 0);
        
        if (c.initial_solved !== undefined) {
            c.progress = Math.max(0, currentSolved - c.initial_solved);
        }
...
function populateFriendSelect() {
    ...
    const myFriends = allLeetcodeData.filter(u => userData.friends.includes(u.username));
    select.innerHTML = myFriends.map(u => `<option value="${u.username}">@${u.username}</option>`).join('');
    if (myFriends.length === 0) select.innerHTML = '<option value="">No friends added yet</option>';
}
...
document.getElementById('createChallengeConfirmBtn').addEventListener('click', async () => {
    ...
    if (challengeFriend && !friendUsername) return showToast('Please select a friend', 'error');

    const myProfile = allLeetcodeData.find(u => u.username === userData.my_leetcode_id) || {};
    const t = getTotals(myProfile);
    const initialSolved = difficulty === 'Any' ? t.total : (t[difficulty.toLowerCase()] || 0);

    const challenge = {
        difficulty,
        target,
        progress: 0,
        initial_solved: initialSolved,
        completed: false,
...
async function checkAchievements() {
    if (!userData.my_leetcode_id) return ACHIEVEMENTS;
    const myProfile = allLeetcodeData.find(u => u.username === userData.my_leetcode_id);
    if (!myProfile) return ACHIEVEMENTS;
    ...
    const streak = daysSinceUpdate <= 1 ? Math.floor(xp / 500) + 1 : 0; 
    ...
    try {
        ...
        const earnedIds = computedAchievements.filter(a => a.earned).map(a => a.id);
...
async function renderAchievements() {
    ...
    const displayList = await checkAchievements() || ACHIEVEMENTS;
...
// --- GLOBAL FUNCTIONS & 1v1 COMPARE SYSTEM ---
window.removeFriend = async function(username) {
    if (!confirm(`Are you sure you want to stop tracking @${username}?`)) return;
    try {
        const userRef = doc(db, "users", currentUserUid);
        await updateDoc(userRef, { friends: arrayRemove(username) });
        userData.friends = userData.friends.filter(f => f !== username);
        showToast(`Removed @${username}`, 'success');
        renderAll(); 
    } catch (e) { showToast('Failed to remove friend', 'error'); }
};

window.openCompareModal = function(targetUsername = null) {
...
document.getElementById('compareBtn')?.addEventListener('click', () => {
    ...
    const myT = getTotals(myProfile); const friendT = getTotals(friendProfile);
    const myXP = myProfile.xp || calculateXP(myProfile); const friendXP = friendProfile.xp || calculateXP(friendProfile);
    const myLevel = calculateLevel(myXP); const friendLevel = calculateLevel(friendXP);
    ...
    document.getElementById('compareResult').innerHTML = `
        ...
        <div class="compare-stats">
            ${stats.map(s => {
                const myWin = s.my > s.friend; const tie = s.my === s.friend;
                return `
...
function renderReport() {
    ...
    if (!userData.my_leetcode_id) return container.innerHTML = `<div class="empty"><div class="empty-icon">📊</div><h3>No Data</h3><p>Link your profile to see weekly reports.</p></div>`;
    const myProfile = allLeetcodeData.find(u => u.username === userData.my_leetcode_id);
    if (!myProfile) return container.innerHTML = `<div class="empty"><div class="empty-icon">📊</div><h3>Loading...</h3><p>Fetching your data.</p></div>`;
    ...
    const thisWeekProblems = Math.floor(t.total * 0.15); 
    ...
    container.innerHTML = `
        <div class="report-card">
            ...
            <div class="report-stats-grid">
                <div class="report-stat-box"><div class="report-stat-icon">📝</div><div class="report-stat-value">${thisWeekProblems}</div><div class="report-stat-label">Problems Solved</div></div>
                <div class="report-stat-box"><div class="report-stat-icon">⚡</div><div class="report-stat-value">+${thisWeekProblems * 20}</div><div class="report-stat-label">XP Earned</div></div>
                <div class="report-stat-box"><div class="report-stat-icon">🎯</div><div class="report-stat-value">Lv ${level}</div><div class="report-stat-label">Current Level</div></div>
                <div class="report-stat-box"><div class="report-stat-icon">🏆</div><div class="report-stat-value">${t.total}</div><div class="report-stat-label">Total Solved</div></div>
            </div>
            <div class="report-comparison">
                <div class="report-comparison-item"><div class="report-comparison-value" style="color: var(--text2)">${lastWeekProblems}</div><div class="report-comparison-label">Last Week</div></div>
                <div class="report-comparison-arrow ${improvement >= 0 ? 'up' : 'down'}">${improvement >= 0 ? '→' : '←'}</div>
                <div class="report-comparison-item"><div class="report-comparison-value" style="color: var(--accent)">${thisWeekProblems}</div><div class="report-comparison-label">This Week</div></div>
                <div class="report-comparison-arrow ${improvement >= 0 ? 'up' : 'down'}">${improvement >= 0 ? '↑' : '↓'}</div>
                <div class="report-comparison-item"><div class="report-comparison-value" style="color: ${improvement >= 0 ? 'var(--easy)' : 'var(--hard)'}">${improvement >= 0 ? '+' : ''}${improvementPct}%</div><div class="report-comparison-label">Change</div></div>
            </div>
        </div>
        <div class="report-streak">
            <div class="report-streak-icon">🔥</div>
            <div class="report-streak-info"><div class="report-streak-value">${streak} Days</div><div class="report-streak-label">Current Streak</div></div>
            <div style="text-align: right;"><div style="font-size: 0.7rem; color: var(--text2);">Keep solving daily to grow your streak!</div></div>
        </div>`;
}

function getTotals(user) { const s = user.problems_solved || {}; return { easy: s.Easy||0, medium: s.Medium||0, hard: s.Hard||0, total: (s.Easy||0) + (s.Medium||0) + (s.Hard||0) }; }
function calculateXP(user) { const t = getTotals(user); return (t.easy * 10) + (t.medium * 20) + (t.hard * 30); }
function calculateLevel(xp) { return Math.floor(Math.sqrt(xp / 100)); }
function getUnifiedScore(lcProfile, cfProfile) { let score = 0; if (lcProfile) score += (lcProfile.xp || calculateXP(lcProfile)); if (cfProfile) score += (cfProfile.rating || 0); return score; }
function getLevelProgress(xp) { const level = calculateLevel(xp); const currentLevelXP = level * level * 100; const nextLevelXP = (level + 1) * (level + 1) * 100; const progress = ((xp - currentLevelXP) / (nextLevelXP - currentLevelXP)) * 100; return { level, progress, currentLevelXP, nextLevelXP }; }
function getInitial(user) { return (user.name || user.username || 'U')[0].toUpperCase(); }
function formatRelative(dateStr) {
    if (!dateStr) return '—';
    const diff = Date.now() - new Date(dateStr);
    const mins = Math.floor(diff / 60000); const hrs = Math.floor(diff / 3600000);
    if (mins < 60) return `${mins}m ago`; if (hrs < 24) return `${hrs}h ago`; return `${Math.floor(diff / 86400000)}d ago`;
}
...
document.getElementById('openModalBtn').addEventListener('click', () => { document.getElementById('modal').classList.add('open'); document.getElementById('usernameInput').focus(); });
document.getElementById('cancelBtn').addEventListener('click', closeModal);
document.getElementById('modal').addEventListener('click', (e) => { if (e.target.id === 'modal') closeModal(); });
function closeModal() { document.getElementById('modal').classList.remove('open'); document.getElementById('usernameInput').value = ''; }

document.getElementById('confirmBtn').addEventListener('click', async () => {
    const username = document.getElementById('usernameInput').value.trim();
    if (!username) return showToast('Please enter a username', 'error');
    if (userData.friends.includes(username) || userData.my_leetcode_id === username) return showToast('User is already tracked', 'error');

    const btn = document.getElementById('confirmBtn'); btn.disabled = true; btn.innerHTML = 'Adding...';
    try {
        ...
        } else showToast(d.message || 'Failed to add user', 'error');
    } catch (e) { showToast('Connection error', 'error'); }
    btn.disabled = false; btn.innerHTML = 'Add Friend';
});
...
function showToast(msg, type = 'success') {
    const el = document.getElementById('toast'); document.getElementById('toastMsg').textContent = msg; el.className = `toast ${type} show`;
    clearTimeout(toastTimer); toastTimer = setTimeout(() => el.classList.remove('show'), 3000);
}
function showError(msg) { document.getElementById('profileContent').innerHTML = `<div class="empty"><h3>Error</h3><p>${msg}</p></div>`; }
