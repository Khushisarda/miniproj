...
function renderChallenges() {
    ...
    if (userChallenges.my_challenge) {
        ...
        // NEW: Calculate real progress dynamically 
        const myProfile = allLeetcodeData.find(u => u.username === userData.my_leetcode_id) || {};
        const t = getTotals(myProfile);
...
document.getElementById('createChallengeConfirmBtn').addEventListener('click', async () => {
    ...
    // Capture the current amount of solved problems as a baseline
    const myProfile = allLeetcodeData.find(u => u.username === userData.my_leetcode_id) || {};
    const t = getTotals(myProfile);
...
async function renderAchievements() {
    ...
    // Await the achievement check so progress data exists before rendering
...
window.removeFriend = async function(username) {
    ...
    try {
        ...
        renderAll(); // Re-render everything seamlessly 
    } catch (e) {
...
window.openCompareModal = function(targetUsername = null) {
    ...
    // Auto-select and trigger compare if launched from a friend card
